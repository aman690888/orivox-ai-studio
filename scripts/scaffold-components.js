...
write(
  ...
  baseTemplate(
    "Title",
    `<h1 className="text-[clamp(2.5rem,5cqw,4.5rem)] font-black tracking-tight leading-tight mb-2" style={{ fontFamily: theme?.typography?.headingFont || theme?.typography?.heading || "sans-serif", color: theme?.colors?.textPrimary || theme?.colors?.primary || "#ffffff", textAlign: data.alignment || "left" }}><EditableText slideId={slideId} componentId={componentId} field="content" value={data.content} /></h1>`,
  ),
);
...
const indexContent = folders
  .map((f) => {
    const files = fs
      .readdirSync(path.join(componentsDir, f))
