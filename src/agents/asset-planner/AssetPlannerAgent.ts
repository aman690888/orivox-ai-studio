...
export class AssetPlannerAgent implements IAgent<
  AssetPlannerInput,
  AssetPlanOutput | ClarificationRequest
> {
  ...
  public async execute(
    context: AssetPlannerInput,
    signal: AbortSignal,
  ): Promise<AssetPlanOutput | ClarificationRequest> {
  ...
  private parse(rawPayload: any): AssetPlanOutput {
    const assets: DetailedAssetPlan[] = Array.isArray(rawPayload?.assets)
      ? rawPayload.assets.map((ast: any) => ({
          ...
          generation_required:
            typeof ast.generation_required === "boolean" ? ast.generation_required : true,
          external_source_allowed:
            typeof ast.external_source_allowed === "boolean" ? ast.external_source_allowed : false,
          citation_required:
            typeof ast.citation_required === "boolean" ? ast.citation_required : false,
          ...
          rendering_priority:
            typeof ast.rendering_priority === "number" ? ast.rendering_priority : 5,
          validation_metadata:
            typeof ast.validation_metadata === "object" ? ast.validation_metadata : {},
  ...
  private validate(
    ...
    if (plan.global_confidence_score < this.MIN_CONFIDENCE_THRESHOLD) {
      return {
        clarificationRequired: true,
        questions: [
          "Visual asset constraints clash with the selected layouts. Can we simplify the charts or diagrams?",
        ],
        missingFields: [],
      };
    }
    ...
    plan.assets.forEach((asset) => {
      ...
      if (assetIds.has(asset.asset_id)) {
        throw new Error(
          `[AssetPlannerAgent] Validation Error: Duplicate asset_id '${asset.asset_id}'.`,
        );
      }
      ...
      if (!validSlideIds.has(asset.slide_id)) {
        throw new Error(
          `[AssetPlannerAgent] Validation Error: Asset '${asset.asset_id}' references unknown slide_id '${asset.slide_id}'.`,
        );
      }
      ...
      if (!allowedComps) {
        throw new Error(
          `[AssetPlannerAgent] Validation Error: Slide '${asset.slide_id}' has no components mapped, cannot own asset '${asset.asset_id}'.`,
        );
      }
      if (!allowedComps.has(asset.owning_component_id)) {
        throw new Error(
          `[AssetPlannerAgent] Validation Error: Asset '${asset.asset_id}' references non-existent component '${asset.owning_component_id}' on slide '${asset.slide_id}'.`,
        );
      }
    });
