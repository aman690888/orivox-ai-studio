...
describe("Performance Stress Tests", () => {
  it("handles compiling a 100-slide presentation efficiently", () => {
    ...
    for (let i = 0; i < numSlides; i++) {
      ...
      layoutPlanLayouts.push({
        slide_id: slideId,
        selected_layout_id: "l1",
        slot_mappings: [],
        rationale: "b",
      });
      ...
      for (let j = 0; j < componentsPerSlide; j++) {
        ...
        placeholders.push({
          placeholder_id: `p_${i}_${j}`,
          owning_component_id: compId,
          content_type: "paragraph",
          requirement_description: "x",
        });
        populated.push({ placeholder_id: `p_${i}_${j}`, value: `Paragraph content ${j}` });

        if (j % 5 === 0) {
          // Every 5th component gets an asset (Total 1000 assets)
          assets.push({
            asset_id: `a_${i}_${j}`,
            slide_id: slideId,
            owning_component_id: compId,
  ...
  it("handles 100 surgical single-slide recompilations fast", () => {
    ...
    const input: CompilerInput = {
      ...
      componentPlan: {
        slides: [{ slide_id: "s1", component_tree: [{ id: "c1", type: "Title", children: [] }] }],
      } as any,
      contentPlan: {
        slides: [
          {
            slide_id: "s1",
            placeholders: [
              { placeholder_id: "p1", owning_component_id: "c1", content_type: "title" },
            ],
          },
        ],
      } as any,
      contentOutput: {
        slides: [
          { slide_id: "s1", populated_placeholders: [{ placeholder_id: "p1", value: "New" }] },
        ],
      } as any,
      assetPlan: { assets: [] } as any,
    };
