...
export const Image: React.FC<BaseComponentProps> = ({
  ...
  <div
    ...
    style={{
      WebkitMaskImage: data.gradientMask
        ? `linear-gradient(${data.gradientMaskDirection || "to bottom"}, black 40%, transparent 100%)`
        : undefined,
      maskImage: data.gradientMask
        ? `linear-gradient(${data.gradientMaskDirection || "to bottom"}, black 40%, transparent 100%)`
        : undefined,
    }}
  >
    <img
      src={
        (data.asset_id && assets?.[data.asset_id]?.url) ||
        data.url ||
    ...
    {data.caption && (
      <div className="absolute bottom-0 inset-x-0 bg-black/60 backdrop-blur-md p-3">
        <p className="text-[clamp(0.75rem,1.25cqw,1rem)] text-center opacity-90 italic text-white">
          {data.caption}
        </p>
      </div>
    )}
