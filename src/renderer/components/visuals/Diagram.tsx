...
export const Diagram: React.FC<BaseComponentProps> = ({
  data,
  theme,
  slideId,
  componentId,
  ...
  return (
    <div
      className="w-full rounded-3xl border p-[clamp(1.5rem,3cqw,2rem)] flex flex-col items-center justify-center my-4 shadow-2xl backdrop-blur-xl"
      style={{
    ...
    >
      ...
      <h4
        className="font-mono text-[clamp(0.7rem,1.2cqw,0.875rem)] font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-4 shadow-inner border border-indigo-500/30"
        style={{
          backgroundColor: theme?.colors?.badgeBg || "rgba(99, 102, 241, 0.15)",
          color: theme?.colors?.primary || "#818cf8",
        }}
      >
      ...
      <div ref={containerRef} className="w-full overflow-auto flex justify-center p-4">
        <pre
          className="text-[clamp(0.75rem,1.5cqw,0.875rem)] font-mono p-6 rounded-2xl text-left overflow-auto w-full max-h-64 whitespace-pre-wrap border border-white/10 shadow-inner text-emerald-400"
