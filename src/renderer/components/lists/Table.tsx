...
export const Table: React.FC<BaseComponentProps> = ({
  ...
  <div
    className="overflow-hidden rounded-3xl border w-full shadow-2xl my-4 backdrop-blur-lg"
    style={{
      backgroundColor: theme?.colors?.cardBg || "rgba(24, 24, 27, 0.4)",
        ...
        <thead>
          <tr style={{ backgroundColor: theme?.colors?.badgeBg || "rgba(99, 102, 241, 0.15)" }}>
            {(data.headers || []).map((h: string, i: number) => (
              <th
                key={i}
                className="p-[clamp(1rem,2cqw,1.25rem)] font-black border-b border-white/10 uppercase tracking-widest text-[clamp(0.7rem,1.2cqw,0.875rem)] text-indigo-200"
              >
                <EditableText
                  slideId={slideId}
