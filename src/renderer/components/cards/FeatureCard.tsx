...
export const FeatureCard: React.FC<BaseComponentProps> = ({
  data,
  theme,
  slideId,
  componentId,
  ...
  >
    <div
      className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-bl-full -mr-16 -mt-16 blur-2xl pointer-events-none"
      style={{ backgroundColor: theme?.colors?.primary || "#6366f1", opacity: 0.1 }}
    ></div>
    <div className="relative z-10">
      <h3 className="text-[clamp(1.25rem,2.2cqw,1.75rem)] font-bold mb-3 text-white drop-shadow-sm">
        {data.title}
      </h3>
      <p className="text-[clamp(0.875rem,1.5cqw,1.125rem)] opacity-80 leading-relaxed text-zinc-300">
        {data.description}
      </p>
    </div>
  </motion.div>
