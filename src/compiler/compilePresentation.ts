...
export function compilePresentation(input: CompilerInput): ValidatedIR {
  const presentationId =
    input.presentation_id || `pres-${Math.random().toString(36).substring(2, 9)}`;
  ...
  input.assetPlan.assets.forEach((asset) => {
    ...
    if (asset.asset_type === "image") {
      ...
      const spec = ImageEngine.generateImageSpec(
        slidePlan?.slide_purpose || "Visual",
        input.intent?.topic?.value || "Topic",
      );
    ...
    if (
      ...
      assetsDict[asset.asset_id] = {
        ...
        type:
          asset.asset_type === "image"
            ? "image"
            : asset.asset_type === ("animation" as any)
              ? "video"
              : "icon",
        url: url,
        generation_prompt: asset.image_style
          ? `${asset.image_style} ${asset.composition}`
          : undefined,
      };
    }
  });
  ...
  const selectedTheme =
    ThemeEngine.getTheme(input.director?.theme_id) ||
