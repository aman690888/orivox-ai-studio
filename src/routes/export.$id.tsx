    ...
    <div className="flex min-h-screen flex-col bg-background">
      <header className="flex h-12 items-center justify-between border-b border-border px-4">
        <div className="flex items-center gap-3">
          <Link to="/present/$id" params={{ id }} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
      ...
      <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col justify-center px-6 py-10">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Pick a format. Orivox handles the rest.</p>
        ...
        <div className="mt-6 grid gap-2 md:grid-cols-3">
          {(
            ...
            <button
              key={f.id}
              onClick={() => { setFmt(f.id); setState("idle"); setCopied(false); }}
              className={`glass rounded-2xl p-4 text-left transition ${
                fmt === f.id ? "border-electric/60 ring-1 ring-electric/50" : "hover:border-white/20"
              }`}
            >
          ...
          <AnimatePresence mode="wait">
            {state === "idle" && (
              <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
                ...
                <button onClick={start} className="rounded-lg bg-foreground px-4 py-2 text-sm font-medium text-background transition hover:opacity-90">
                  {fmt === "link" ? "Generate link" : `Export as ${fmt.toUpperCase()}`}
                </button>
              </motion.div>
            )}
            {state === "working" && (
              <motion.div key="working" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
                ...
                <div className="mx-auto mt-3 h-1 w-40 overflow-hidden rounded-full bg-white/5">
                  <motion.div className="h-full bg-electric" initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ duration: 1.4 }} />
                </div>
              </motion.div>
            )}
            {state === "done" && (
              <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full text-center">
                ...
                {fmt === "link" ? (
                  <div className="mt-4 flex items-center gap-2 rounded-lg border border-border bg-white/[0.02] p-2 text-xs">
                    <span className="flex-1 truncate px-2 font-mono text-muted-foreground">orivox.app/p/{id}</span>
                    <button
                      onClick={() => { navigator.clipboard?.writeText(`https://orivox.app/p/${id}`); setCopied(true); }}
